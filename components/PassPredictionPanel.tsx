    ...
    <div className="text-sm">
      <div className="px-4 py-2.5 border-b border-cyan-500/10">
        <span className="text-xs font-semibold uppercase tracking-wider text-cyan-400/80">
          Upcoming passes · 24 h
        </span>
      </div>

      <div className="px-4 py-3 space-y-2" style={{ contentVisibility: 'auto' }}>
        {!hasTle ? (
          <p className="py-5 text-slate-500 text-center text-xs">
          ...
          <div className="flex items-center justify-center gap-2 py-6 text-slate-400 text-xs">
            <span
              className="inline-block h-4 w-4 rounded-full border-2 border-cyan-400/30 border-t-cyan-400 animate-spin"
              aria-hidden
            />
            Computing passes…
          </div>
          ...
          passes?.map((pass, i) => (
            <div
              ...
              className="bg-black/20 border border-cyan-500/10 rounded-lg px-3 py-2"
            >
      ...
      {hasTle && passes && passes.length > 0 && (
        <div className="px-4 py-2 text-slate-500 text-[10px] text-right font-mono">
